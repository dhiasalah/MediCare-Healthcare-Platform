...
interface PrescribeMedicamentModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSuccess: () => void;
  patientId: number;
...
export function PrescribeMedicamentModal({
  isOpen,
  onClose,
  onSuccess,
  patientId,
}: PrescribeMedicamentModalProps) {
  ...
  const onSubmit = async (data: MedicamentFormData) => {
    ...
    try {
      await api.post("/api/patients/medicaments/", {
        ...data,
        patient: patientId,
        status: "active",
      });
      showToast.success("Médicament prescrit avec succès");
      onSuccess();
      onClose();
      reset();
    } catch (error) {
      console.error("Error prescribing medicament:", error);
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <DialogDescription className="text-base mt-1">
                Ajoutez un nouveau médicament au traitement du patient
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
